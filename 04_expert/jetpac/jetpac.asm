...
gfx_params_platforms:
  ...
  defb $06,$78,$b8,$88    ; Bottom (ground)
...
PlayerInit:
  ...
  ld de,jetman_motion     ; Jetman object
  ld bc,$0008
  ldir
...
;
; Used by the routine at PlayerTurnStarts.
ScoreLabelFlash:
...
main_jump_table:
  ...
  defw PlayerTurnStarts   ; Player Turn Starts + Jetman Fly
  defw JetmanWalk         ; Jetman Walk
  defw MeteorUpdate       ; Meteor Update
  defw ItemPickup         ; Collision Detection
...
JetFighterUpdate_6:
  ...
  call PlatformCollisionCheckAll ; Platform collision - returns E
...
ItemCheckCollect:
  call TmpActorPosUpdate  ; Update Actor position direction
  call PlatformCollisionCheckAll ; Platform collision - returns E
...
; Item pickup: collectibles/rocket modules.
;
; Input:IX Collectible item object.
ItemPickup:
  call TmpActorPosUpdate  ; Update Actor position direction
  ld a,(ix+$04)           ; A=Jetman state
...
; Redraw a sprite.
;
; Used by the routines at ItemPickup, CollectRocketItem and CarryRocketItem.
RedrawSprite:
...
;
; Used by the routine at ItemPickup.
GetCollectibleID:
...
CollectRocketItem:
  set 1,(ix+$04)          ; Set Jetman state to be carrying
  ...
  ld (ix+$02),h           ;
  call TmpActorPosUpdate  ; Update sprite X position
...
; sprite. Used by the routine at ItemPickup.
;
; Input:IX Rocket module object.
CarryRocketItem:
  ld hl,(jetman_pos_x)    ; Update module position so it becomes attached to
...
; Used by the routine at NewActor.
ItemNewCollectible:
  ld a,(jetman_motion)    ; Jetman direction
  and $3f
...
RocketAnimateFlames:
  ...
  add a,$15               ;
  ld (hl),a               ;
  ld a,(ix+$02)              ; If on the ground, turn off flame sprites
  cp $b8                     ;
...
RocketUpdate:
  ...
  call TmpActorPosUpdate  ; Update Jetman position direction
...
;
; Used by the routines at ItemCheckCollect and PlayerTurnStarts.
SfxPickupItem:
  ld d,$30                ; Pitch
  ld c,$40                ; Duration
...
ExplosionAfterKill:
  ...
  ld hl,jetman_motion     ; Jetman object
  ...
  pop ix
  xor a
  ld (jetman_motion),a    ; Reset Jetman direction to not moving
  ret
...
NewActor_1:
  ...
  and a                           ; counter > zero
  jr nz,NewActor_4                ;
  ld a,(jetman_motion)    ; Get jetman direction
  and $3f
  dec a
...
; Alien Collision.
;
; Used by the routines at JetFighterUpdate, ItemCheckCollect, ItemPickup,
; RocketUpdate, SquidgyAlienUpdate, UFOUpdate, SphereAlienUpdate,
; CrossedShipUpdate and MeteorUpdate.
;
;  Input:IX Alien object.
; Output:E was alien hit: 0=no hit, 1=hit!
AlienCollision:
  ld hl,jetman_motion     ; HL=Jetman object
...
LaserBeamInit:
  ...
  ld de,jetman_motion     ; DE=Jetman object
...
; Copy an actors position data on to the temporary Actor object.
;
; Used by the routines at ItemCheckCollect, ItemPickup, CollectRocketItem,
; RocketTakeoff, RocketLanding, RocketUpdate, AnimateExplosion, ActorUpdateDir,
; MeteorUpdate, JetmanFlyThrust and JetmanWalk.
;
...
;
; Used by the routines at JetmanFalling and JetmanWalk.
;
...
; Player turn starts, which also checks Jetman thrust input.
PlayerTurnStarts:
...
PlayerTurnStarts_1:
  ...
  jr PlayerTurnStarts_0   ;

; Airborne Jetman update.
;
; Read thrust/direction controls and update position accordingly. Used by the
; routine at PlayerTurnStarts.
;
; Input:IX Jetman object.
JetmanFlyThrust:
  call TmpActorPosUpdate  ; Update actor position direction on screen
  call ReadInputLR        ; Read Left/Right input
  bit 3,a                 ; Jetman thrust right, update velocity
  jp z,JetmanFlyThrust_0  ;
  bit 4,a                 ; Jetman thrust left, update velocity
  jp z,JetmanFlyLeft      ;
  ld a,(game_timer)       ; Game timer
  and $01                  ; If bit-0 is reset, update horizontal velocity
  jr z,JetmanFlyHorizontal ;
  jp JetmanVelCalcX       ; Calculate new horizontal velocity
JetmanFlyThrust_0:
  res 6,(ix+$00)          ; Set Jetman direction to be "right"
  bit 6,(ix+$04)          ; Flip direction, with velocity
  jp nz,JetmanDirFlipX    ;

; Increase Jetman horizontal velocity.
;
; Used by the routine at JetmanFlyLeft.
;
; Input:IX Jetman object.
JetmanVelIncX:
  ld a,(jetman_velocity_modifier) ; Jetman velocity modifier ($00 or $04)
  neg
  add a,$08
  add a,(ix+$05)          ; A += Jetman X velocity
  cp $40
  jr nc,JetmanVelSetXMax  ; Jump if velocity >= max

; Update Jetman X velocity with new value.
;
; Used by the routines at JetmanDirFlipX and JetmanVelCalcX.
JetmanVelSetX:
  ld (ix+$05),a           ; Update Jetman X velocity with A (will be < 64)
  jr JetmanFlyHorizontal  ; Fly horizontally

; Set Jetman X velocity to the max flying speed.
;
; Used by the routine at JetmanVelIncX.
JetmanVelSetXMax:
  ld (ix+$05),$40

; Fly Jetman horizontally.
;
; Used by the routines at JetmanFlyThrust, JetmanVelSetX and JetmanDirFlipX.
;
;  Input:IX Jetman object.
...
JetmanFlyHorizontal:
  ld h,$00
  ld l,(ix+$05)           ; Jetman X velocity (will be <= 64)
  add hl,hl               ; Multiply X velocity by 3
  add hl,hl               ;
  add hl,hl               ;
...
;
; Used by the routine at JetmanFlyDecX.
;
; Input:IX Jetman object.
;       H New X position.
;       L New Thrust value.
...
JetmanApplyGravity_0:
  ...
  and $1f
  cp $1f                  ; Check if any keys on the row are pressed
  jr nz,JetmanVelSetYMin  ; Jump if so - hover Jetman
  ld a,$f7                ; Read top-left row of keys
  djnz JetmanApplyGravity_0 ; ...and repeat

; Check if Jetman is falling downward.
;
; Used by the routine at JetmanFlyCheckThrust.
JetmanFalling:
  call ReadInputThrust    ; Check if THRUST button pressed (Joystick or Keys)
  bit 1,a                  ; Set Jetman to down position if not thrusting
  jp nz,JetmanStateSetDown ;
  res 7,(ix+$00)          ; Set Jetman direction to RIGHT
  bit 7,(ix+$04)          ;
  jp nz,JetmanDirFlipY    ; Flip vertical direction if moving down

; Increase Jetman vertical velocity.
;
; Used by the routine at JetmanStateSetDown.
JetmanVelIncY:
  ld a,(jetman_velocity_modifier) ; Jetman velocity modifier ($00 or $04)
  neg
  add a,$08
  add a,(ix+$06)          ; A += Jetman Y velocity
  cp $3f
  jr nc,JetmanVelSetYMax  ; Set vertical velocity to max if >= 63

; Update Jetman vertical velocity with new value.
;
; Used by the routine at JetmanDirFlipY.
JetmanVelSetY:
  ld (ix+$06),a           ; Y velocity = A (is < 63)
  jr JetmanFlyVertical    ; Update vertical flying

; Set Jetman vertical velocity to zero.
;
; Used by the routines at JetmanApplyGravity and JetmanFlyCheckThrust.
JetmanVelSetYMin:
  ld (ix+$06),$00         ; Jetman Y velocity is zero
  jr JetmanFlyVertical    ; Update vertical flying
...
JetmanFlyCheckThrust:
  ...
  bit 2,a                 ; Set Y velocity to zero if not thrusting
  jp z,JetmanVelSetYMin   ;
  jr JetmanFalling        ; else check if falling and update movement

; Set Jetman vertical velocity to maximum.
;
; Used by the routine at JetmanVelIncY.
JetmanVelSetYMax:
  ld (ix+$06),$3f         ; Set Jetman Y velocity to 63

; Fly Jetman vertically.
;
; Used by the routines at JetmanVelSetY, JetmanVelSetYMin and JetmanDirFlipY.
;
; Input:IX Jetman object.
JetmanFlyVertical:
  ld l,(ix+$06)           ; L=Jetman Y velocity (will be <= 63)
  ld h,$00
  add hl,hl               ; Multiply velocity by 3
  add hl,hl               ;
  add hl,hl               ;
  ld d,(ix+$02)           ; D=Jetman Y position
  ld a,(jetman_fly_counter) ; E=Jetman flying counter
  ld e,a                    ;
  bit 7,(ix+$04)          ; Move Jetman up if state is up
  jp z,JetmanFlyUp        ;
  add hl,de               ; else move downwards

; Check vertical position while in mid-flight.
;
; Used by the routine at JetmanFlyUp.
JetmanFlyCheckY:
  ...
  ld a,h
  cp $c0
  jr nc,JetmanStateSetUp  ; Move up if within screen limits: 42 to 192
  cp $2a
  jr c,JetmanStateSetDownSlow ; Has reached top of screen

; Jetman: check if collides with a platform.
;
; Used by the routines at JetmanStateSetUp and JetmanStateSetDownSlow.
;
; Input:IX Jetman object.
JetmanPlatformCollision:
  call PlatformCollisionCheckAll ; Platform collision detection (returns E)
...
;
; Used by the routine at JetmanPlatformCollision.
JetmanBumpsHead:
...
JetmanLands:
  ...
  ld (ix+$05),$00         ; Set Jetman X velocity to stopped
  ld (ix+$06),$00         ; Set Jetman Y velocity to stopped
  jr JetmanRedraw         ; Redraw Jetman

; Reset Jetman movement direction.
;
; Used by the routine at JetmanFlyCheckY.
JetmanStateSetUp:
  res 7,(ix+$04)          ; Set Jetman to be moving up
  jr JetmanPlatformCollision ; Jetman flight collision detection

; Slow Jetman's velocity and change direction state to down.
;
; Used by the routine at JetmanFlyCheckY.
JetmanStateSetDownSlow:
  set 7,(ix+$04)          ; Set Jetman to be moving down
  ld a,(ix+$06)           ; Jetman Y velocity
  srl a                   ; Divide by 2
  jr z,JetmanPlatformCollision ; Check platform collision if A was $00 or $01
  ld (ix+$06),a           ; else update Jetman Y velocity
  jr JetmanPlatformCollision ; Check for platform collision

; Jetman is falling.
;
; Used by the routine at JetmanFalling.
JetmanStateSetDown:
  set 7,(ix+$00)          ; Set Jetman direction to WALK/DOWN
  bit 7,(ix+$04)          ; Increment Jetman Y velocity if moving down
  jp nz,JetmanVelIncY     ;
...
JetmanDirFlipY:
  ...
  sub $08                 ; A=$F8 or $FC
  add a,(ix+$06)          ; A += Jetman Y velocity
  jp p,JetmanVelSetY      ; Update vertical velocity if new velocity is
                          ; positive
  ld (ix+$06),$00         ; else set Y velocity to zero
...
JetmanFlyDecX:
  ...
  ex de,hl
  sbc hl,de
  jp JetmanApplyGravity   ; Update Jetman velocity/dir if thrusting
...
JetmanFlyLeft:
  ...
  bit 6,(ix+$04)          ; Increase Jetman X velocity if moving RIGHT
  jp nz,JetmanVelIncX     ;
...
JetmanDirFlipX:
  ...
  sub $08                 ; A=$F8 or $FC
  add a,(ix+$05)          ; A += Jetman X velocity
  jp p,JetmanVelSetX      ; Update horizontal velocity if new velocity is
                          ; positive
  ld (ix+$05),$00         ; else set X velocity to zero
...
JetmanFlyUp:
  ...
  jp JetmanFlyCheckY      ; Check vertical position within screen limits

; Calculate new Jetman horizontal velocity.
;
; Used by the routine at JetmanFlyThrust.
JetmanVelCalcX:
  ld a,(jetman_velocity_modifier) ; Jetman velocity modifier ($00 or $04)
  ...
  jp p,JetmanVelSetX      ; Update horizontal velocity if new velocity is
                          ; positive
  xor a
  jp JetmanVelSetX        ; Update horizontal velocity to zero
...
JetmanWalk:
  ...
  bit 4,a                 ; Walk LEFT
  jp z,JetmanWalk_4       ;
  ld (ix+$05),$00         ; else set Jetman X velocity to zero
JetmanWalk_0:
  call ReadInputThrust    ; Read THRUST button
  bit 1,a                  ; Walk off platform if thrusting
  jr z,jetmanLeavePlatform ;
  call PlatformCollisionCheckAll ; Platform collision check (returns E)
  bit 2,e                  ; Leave platform if bit-2 is reset
  jr z,jetmanLeavePlatform ;
  bit 3,e                 ; Redraw Jetman if bit-3 is reset
  jp z,JetmanRedraw       ;
  ld a,(ix+$05)           ; Jetman X velocity
  and a
  jp nz,JetmanRedraw      ; Redraw Jetman if X velocity > 0
  ...
  jp JetmanRedraw         ; routine is called. Note also that the X velocity is
JetmanWalk_1:
  inc (ix+$01)            ; set to max walking velocity, but this instruction
  ld (ix+$05),$20         ; might be irrelevant as hacking the velocity value
  jp JetmanRedraw         ; has no noticeable effect.
...
JetmanWalk_3:
  ...
  ld (ix+$05),$20         ; Set Jetman X velocity to maximum
...
; Related to vertical platform collision detection...needs more work!
;
; Used by the routine at platformCollisionForActor.
;
; Input:A Sprite X position minus Platform X position?
;       IX Jetman or alien object
PlatformCollisionVertical:
  ex af,af'
  ld a,(ix+$00)           ; actor direction
  and $3f                 ; Value must be <= 63
  cp $03                           ; Jump if value == 3 (therefore never when
  jr z,PlatformCollisionVertical_0 ; Jetman)
  ex af,af'               ; Restore A to entry value
  jr PlatformCollision    ; Vertical collision detection
PlatformCollisionVertical_0:
  ex af,af'               ; Flip again
  sub $09
  jp p,PlatformCollision  ; Vertical collision detection, first incrementing by
  add a,$09               ; $09 if A is negative
  jr PlatformCollision    ;

; Iterate over all platforms and check for a collision.
;
...
; routines at JetFighterUpdate, ItemCheckCollect, ItemPickup,
; SquidgyAlienUpdate, UFOUpdate, SphereAlienUpdate,
; CrossedShipPlatformCollision, MeteorUpdate, JetmanPlatformCollision and
; JetmanWalk.
;
;  Input:IX Jetman or Alien object.
; Output:E Collision state.
PlatformCollisionCheckAll:
  ld b,$04                ; Loop counter (4 platform to check)
  ld hl,gfx_params_platforms ; Platform location/size params

; Check if an actor collides with a platform.
;
; Used by the routine at PlatformCollision.
;
; Input:HL Address of platform object.
;       IX Jetman or Alien object.
platformCollisionForActor:
  ld e,$00
  push hl
  inc hl                  ; A=Platform X position
  ld a,(hl)               ;
  sub (ix+$01)            ; Subtract Jetman/Alien X position
  jp p,PlatformCollisionVertical ; If > 0 do XY collision detection
  neg
...
PlatformCollision:
  ...
  cp (hl)                   ; X position >= width (no collision) so try next
  jp nc,PlatformCollision_3 ; platform
  add a,$12
  cp (hl)                  ; Set bit 3 if collision: X position >= width
  jp c,PlatformCollision_0 ;
  set 3,e                  ;
PlatformCollision_0:
  ...
  inc a                   ; Add 2
  inc a                   ;
  jp m,PlatformCollision_3 ; Next platform if no collision: Y position is
                           ; negative
  cp $02                   ; A < 2, Landed on platform
  jr c,PlatformCollision_2 ;
  cp (ix+$07)              ; A < sprite height, hits underneath of platform?
  jr c,PlatformCollision_1 ;
  dec a                   ; Subtract 2
  dec a                   ;
  cp (ix+$07)               ; Next platform if no collision: Y position >=
  jr nc,PlatformCollision_3 ; sprite height
...
PlatformCollision_3:
  ...
  inc hl                  ;
  inc hl                  ;
  djnz platformCollisionForActor ; Loop back until all 4 platforms have been
                                 ; checked
  ret
