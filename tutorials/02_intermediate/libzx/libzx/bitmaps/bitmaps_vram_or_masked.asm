...
draw_bitmap_OR_blitted_masked_vram:
	push hl
	push bc
	push af
	push de
	...
	ld ixh, a
	ld a, (de)
	call draw_line_OR_blitted_masked_vram
	pop de
	pop af
	pop bc
	pop hl
	...
	dec a
	cp 0
	jp nz, draw_bitmap_OR_blitted_masked_vram
...
;     IXH - line data
;       A - mask data
draw_line_OR_blitted_masked_vram:
	push af
	...
	jp z, spans_only_one_square_OR_masked_vram	; when x coordinate is a multiple of 8,
...
shift_line_data_left_loop_OR_masked_vram:
	sla c			; shift line data
	sla e			; shift mask
	dec a
	jp nz, shift_line_data_left_loop_OR_masked_vram
...
spans_only_one_square_OR_masked_vram:
	pop af			; A := mask data
