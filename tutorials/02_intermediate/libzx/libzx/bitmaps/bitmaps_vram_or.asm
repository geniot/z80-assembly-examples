...
; WRITES TO VRAM DIRECTLY!
;
; Draws an 8xA image additively, using a blitting method of OR.
...
;       A - lines count
draw_bitmap_OR_blitted_vram:
	push hl
	push bc
	push af
	ld a, (hl)
	ld ixh, a
	call draw_line_OR_blitted_vram
	pop af
	pop bc
	pop hl
	...
	dec a
	cp 0
	jp nz, draw_bitmap_OR_blitted_vram
...
draw_line_OR_blitted_vram:
	...
	inc hl
	ld e, (hl)		; DE := word (HL)
	ex de, hl		; HL := video_memory_Y_lookup_vram[C]
	...
	jp z, spans_only_one_square_OR_vram	; when x coordinate is a multiple of 8,
...
shift_line_data_left_loop_OR_vram:
	sla c
	dec a
	jp nz, shift_line_data_left_loop_OR_vram
...
spans_only_one_square_OR_vram:
	ld a, ixh		; A := line data
	or (hl)			; A := line data OR existing 8-pixel horizontal data
	ld (hl), a		; put pixel data into square
	ret

	
; Draw a tiled rectangular region in the video ram using OR blitting
;
; 		C - screen Y coordinate
;	  IXL - screen X coordinate
;      HL - pointer to 8 bytes sprite data
...
repeat_tile_OR_vram:
repeat_tile_OR_vram_vertical:
	push bc
	push ix
	push de
	
repeat_tile_OR_vram_horizontal:
	push de
	push bc
	push hl
	...
	ld a, d
	cp 0
	jp nz, repeat_tile_OR_vram_horizontal
	; END INNER LOOP
	...
	ld a, e
	cp 0
	jp nz, repeat_tile_OR_vram_vertical
