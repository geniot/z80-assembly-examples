...
;	
; Draws an 8xA image, using a blitting method of XOR. Any set pixel in the
; input will flip the existing pixel.
; Each resulting pixel's value is calculated as:
;          pixel = input XOR existing
; Input:	
; 		C - screen Y coordinate
;	  IXL - screen X coordinate
;      HL - pointer to 8 bytes sprite data
;       A - lines count
draw_bitmap_XOR_blitted_vram:
	push hl
	push bc
	push af
	ld a, (hl)
	ld ixh, a
	call draw_line_XOR_blitted_vram
	pop af
	pop bc
	pop hl
	...
	dec a
	cp 0
	jp nz, draw_bitmap_XOR_blitted_vram
...
; Blitting mode is XOR.
;
; Input:	
; 		C - screen Y coordinate
;	  IXL - screen X coordinate
;     IXH - line data
draw_line_XOR_blitted_vram:
	...
	jp z, x_spans_only_one_square_vram ; when x coordinate is a multiple of 8,
...
x_shift_line_data_left_loop_vram:
	sla c
	dec a
	jp nz, x_shift_line_data_left_loop_vram
