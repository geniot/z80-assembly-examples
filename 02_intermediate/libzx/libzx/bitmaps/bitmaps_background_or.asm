...
;       A - lines count
draw_bitmap_OR_blitted_background:
	push hl
	push bc
	push af
	ld a, (hl)
	ld ixh, a
	call draw_line_OR_blitted_background
	pop af
	pop bc
	pop hl
	...
	dec a
	cp 0
	jp nz, draw_bitmap_OR_blitted_background
...
;	  IXL - screen X coordinate
;     IXH - line data
draw_line_OR_blitted_background:
	...
	jp z, spans_only_one_square	; when x coordinate is a multiple of 8,
...
shift_line_data_left_loop:
	sla c
	dec a
	jp nz, shift_line_data_left_loop
...
; Draw a tiled rectangular region in the background buffer using OR blitting
;
; 		C - screen Y coordinate
;	  IXL - screen X coordinate
;      HL - pointer to 8 bytes sprite data
...
repeat_tile_OR_background:
repeat_tile_OR_background_vertical:
	push bc
	push ix
	push de
	
repeat_tile_OR_background_horizontal:
	push de
	push bc
	push hl
	...
	ld a, d
	cp 0
	jp nz, repeat_tile_OR_background_horizontal
	; END INNER LOOP
	...
	ld a, e
	cp 0
	jp nz, repeat_tile_OR_background_vertical
